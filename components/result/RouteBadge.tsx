...
interface RouteBadgeProps {
  ...
  /** 'xs'는 min-h-11 없이 컴팩트하게 그린다(예: 통합 시간이력 노선 필터줄) — 터치 영역은
   * touch-target::after(-10px 확장)로 여전히 44px 이상 확보된다. */
  size?: 'xs' | 'sm' | 'md'
...
export default function RouteBadge({ route, selected = false, onClick, size = 'md', disabled = false }: RouteBadgeProps) {
  ...
  const sizeClass = size === 'xs' ? 'text-xs px-2.5 py-1' : size === 'sm' ? 'text-xs px-2 py-1' : 'text-sm px-2.5 py-1.5'
  ...
  return (
    <button
      ...
      className={`touch-target ${size === 'xs' ? '' : 'min-h-11'} inline-flex items-center gap-1 rounded-full border font-bold whitespace-nowrap ${sizeClass} ${
